...
2. Antidote (active)
   - Remove all poison stacks from the player (does not remove poison-causing statuses).
3. Gas Mask (status, charges)
   - Prevent the next X poison applications to the player (1 charge per application), then destroy itself.
4. Extra Filter (active)
   - Refresh charges of an existing Gas Mask.
5. Poison Drink (active)
   - Apply a large amount of stacks (tick is still flat per turn).
